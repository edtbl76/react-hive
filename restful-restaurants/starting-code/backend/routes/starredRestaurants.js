...
router.get("/:id", (req, res) => {
  ...
  // Find restaurant w/ matching id
  const restaurant = STARRED_RESTAURANTS.find((restaurant) => restaurant.id === id);

  // if restaurant doesn't exist let client know it wasn't found
  if (!restaurant) {
    res.sendStatus(404);
    return;
  }
...
router.delete("/:id", (req, res) => {
  ...
  // If length is the same, then user tried to unstar a restuarant that isn't starred
